...
def toaRadianceWV(inImg, metadataFile, doDOS, isPan, sensor):
    ...
    [doesn't anymore after addition of gain and bias] Also works with GeoEye-1
    (and possibly other DigitalGlobe satellties) since they have the same metadata file structure.
    """
    ...
    if sensor == "WV3":
        ...
        if isPan:
            ...
            # as per calibration from DG released 3/6/2015
            gain = [1.157, 1.07, 1.082, 1.048, 1.028, 0.979, 1.006, 0.975]
            # as per calibration from DG released 3/6/2015
            bias = [7.07, 4.253, 2.633, 2.074, 1.807, 2.633, 3.406, 2.258]
    else:
    ...
    for band in range(1,bandNum+1):
        ...
        cond = np.logical_and((rawData-dosDN[band-1]) > 0, rawData != 65536)
        radiometricData[:,:,band-1] = np.where(cond,
                rawData-dosDN[band-1], 0)*absCalFactor[band-1]/effectiveBandwidth[band-1]*(2-gain[band-1])-bias[band-1]
...
def toaRadiancePHR1(inImg, metadataFile, doDOS):
    ...
    for band in range(1,bandNum+1):
        ...
        rawData = np.int_(inImg.GetRasterBand(band).ReadAsArray())
        cond = np.logical_and((rawData-dosDN[band-1]) > 0, rawData != 65536)
        radiometricData[:,:,band-1] = np.where(cond, (rawData-dosDN[band-1])/gain[band-1] + bias[band-1], 0)
        validMask = validMask + radiometricData[:,:,band-1]
...
def toaRadianceL8(inImg, metadataFile, doDOS, isPan, sensor):
    ...
    for band in range(1, rawData.shape[2]+1):
        print(band)
        cond = (rawData[:,:,band-1]-dosDN[band-1]) > 0
        radiometricData[:,:,band-1] = np.where(cond,
                (rawData[:,:,band-1]-dosDN[band-1])*multFactor[band-1] + addFactor[band-1], 0)
        validMask = validMask + radiometricData[:,:,band-1]
